...
namespace DB_LAB
{
    public partial class Search_Ward2 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (con.State == System.Data.ConnectionState.Open)
            {
                if (srch.Text.ToString() != string.Empty)//no value input
                {
                    ...
                    {
                        SqlDataAdapter sqlDa = new SqlDataAdapter("select * from ward where w_Name = '" + srch.Text.ToString() + "' and w_type='Quarantine'", con);
                    ...
                    {
                        SqlDataAdapter sqlDa = new SqlDataAdapter("select * from ward where w_ID = " + srch.Text.ToString() + "and w_type='Quarantine'", con);
                        DataTable dtbl = new DataTable();
                        sqlDa.Fill(dtbl);
                        dataGridView1.DataSource = dtbl;
                    }
                    ...
                    {
                        SqlDataAdapter sqlDa = new SqlDataAdapter("select * from ward where wardSec = '" + srch.Text.ToString() + "' and w_type='Quarantine'", con);
                        DataTable dtbl = new DataTable();
                        sqlDa.Fill(dtbl);
                        dataGridView1.DataSource = dtbl;
                    }
